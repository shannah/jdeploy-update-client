 ...
 * relevant system properties). This class calls UpdateClient.requireVersion(requiredVersion, params)
 * and, if the update flow does not cause the process to exit (installer launched), it displays a
 * simple "Hello World" window so you can verify the application continued to run.</p>
 *
 * <p>Notes:
 * - UpdateClient requires the system property {@code jdeploy.app.version} to be present or it will
 *   return immediately without prompting. The launcher's comparison uses
 * * {@code jdeploy.launcher.app.version} (defaults to "0.0.0" when absent).</p>
 *
 * <p>Run hint: Use your IDE's Run action for this class, or run with a Java exec plugin that
 * supports specifying the main class.</p>
 */
public final class ManualUpdateController {
    ...
    public static void main(String[] args) throws IOException {
        ...
        String source = "https://github.com/BrokkAi/brokk";                               // e.g. "" for npm, or "https://github.com/user/repo" for GitHub source
        ...
        String jdeployAppVersion = "0.17.1";               // sets System property jdeploy.app.version (must be present to run checks)
        String jdeployLauncherAppVersion = "0.17.1";       // sets System property jdeploy.launcher.app.version (used for comparison)
        String prereleaseFlag = "false";                  // optional: "true" or "false" for System property jdeploy.prerelease
        // ---------------------------------------------------------------------
        ...
        // Invoke the update check. If UpdateClient launches the installer successfully it may call System.exit(0)

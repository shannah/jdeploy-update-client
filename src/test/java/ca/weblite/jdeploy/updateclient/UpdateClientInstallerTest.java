 ...
 *
 * - macOS-only: verifies findFirstAppBundle() finds a nested .app directory.
 * - Linux-only: verifies decompressGzipInstaller() decompresses a .gz file to a non-empty file.
 *
 * These tests use reflection to access the private helper methods under test and use
 * platform guards (Assume) to skip tests on unsupported OSes.
 */
public class UpdateClientInstallerTest {

    /**
     * macOS-only: create a temp tree containing a nested TestApp.app directory; assert findFirstAppBundle() locates it.
     */
    @Test
    public void testFindFirstAppBundle_mac() throws Exception {
        ...
        try {
            ...
            Assert.assertTrue("Found path should end with TestApp.app", found.getFileName().toString().toLowerCase().endsWith(".app"));
        } finally {
    ...
    /**
     * Linux-only: create a small gzip file in temp and verify decompressGzipInstaller() produces a non-empty decompressed file.
     */
    @Test
